...
class TestLayoutManager(unittest.TestCase):
    ...
    def test_initial_calculation(self) -> None:
        ...
        self.assertEqual(
            self.layout_manager.separator_rect.x,
            expected_column_width
        )
        self.assertEqual(
            self.layout_manager.separator_rect.width,
            display.AREA_SEPARATOR
        )
    ...
    def test_resize_handling(self) -> None:
        ...
        self.assertEqual(self.layout_manager.command_area.width, expected_new_column_width)
        self.assertEqual(self.layout_manager.command_area.height, new_size[1])
        
        # Check separator position after resize
        self.assertEqual(
            self.layout_manager.separator_rect.x,
            expected_new_column_width
        )
        ...
        self.assertEqual(self.layout_manager.simulation_area.width, expected_new_sim_width)
